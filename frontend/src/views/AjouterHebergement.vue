...
<template>
  <div class="ajouter-hebergement">
    ...
    <form @submit.prevent="submitHebergement">
      ...
      <label>Photos</label>
      <input type="file" multiple @change="handlePhotos" />

      <label>Disponibilités</label>
      <input type="date" v-model="dispoDebut" />
      <input type="date" v-model="dispoFin" />
...
export default {
  ...
  methods: {
    ...
    async submitHebergement() {
      ...
      formData.append("dispo_debut", this.dispoDebut);
      formData.append("dispo_fin", this.dispoFin);
      this.photos.forEach((file, i) => formData.append(`photo_${i}`, file));
